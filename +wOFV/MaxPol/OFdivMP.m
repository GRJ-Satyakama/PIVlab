function div=OFdivMP(V,lD,PD,lS,PS)
%Compute the divergence of a velocity field V using MaxPol 
...
%using independent kernels, which mitigates noise. V is a 2D velocity field
%in optical flow format (u and then v concatenated in dimension 3, u is 
%positive down, v is positive to the right, both in units of pixels). lD, 
%PD, lS, PS are optional parameters passed to MaxPol describing the kernels
%for the differentiation (lD and PD) and smoothing (lS and PS). If left 
%empty, they default to a more-or-less universally effective kernel used by
%McManus in his PhD thesis (McManus, 2019). The output is div, a matrix 
%of the same size as u and v containing the divergence.
...
div=Dx*V(:,:,1)*Sx+Sy*V(:,:,2)*Dy;

%Truncate added boundary points
div=div(5:end-4,5:end-4);