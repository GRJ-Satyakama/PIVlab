function [v1,subOut,minOut,alphafield]=wOFVBayesPyd_run(I0,I1,lambda,options)
...
if ~isfield(options,'MedianFilter')
    MedianFilter.Flag = true;
    MedianFilter.Size = [5,5];
else
    MedianFilter = options.MedianFilter;
end

% Default iamge filtering when building a pyramid
if ~isfield(options,'FiltImg')
    FiltImg.Flag = true;
    FiltImg.Type = 'spline';
else
    FiltImg = options.FiltImg;
end
...
if ~isfield(lambda,'eta')
    eta=10^(-4);
else
    eta=lambda.eta;
end

if ~isfield(lambda,'vartheta')
    vartheta=ones(size(I0));
else
...
if ~isfield(options,'LMat')
    LMat=[]; %GetRegularizationOperator(options.M);
else
    LMat=options.LMat;
end

if ~isfield(options,'OldLambda')
    OldLambda=1;
else
    OldLambda=options.OldLambda;
end
...
for k=1:mx
    ...
    for j=1:my
        ...
        [v1tmp,minOut(domnum),alphatmp]=wOFVBayesPyd_sub(I0sub,I1sub,C,L,eta,varthetaSub,v0sub,...
            regscheme,masksub,Fmats,OldLambda,PyramidLevels,MedianFilter,FiltImg);
