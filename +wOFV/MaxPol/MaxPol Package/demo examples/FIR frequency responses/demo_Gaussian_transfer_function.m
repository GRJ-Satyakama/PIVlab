...
tap_length = 15;
possible_cutoffs = exp(1:-.1:-.7);
side_shifts = [0];
n_ords = [0, 1, 2, 3, 4];
col_numbers = numel(possible_cutoffs);
...
for n_ord = n_ords
    ...
    h_figure{iteration_n} = figure('rend','painters','pos',[n_ord*450, 50, 400, 400]);
    iteration = 0;
    iteration_s = 0;
    iteration_s = iteration_s + 1;
    iteration_sig = 0;
    for sig = possible_cutoffs
        iteration_sig = iteration_sig + 1;
        iteration = iteration + 1;
        l = tap_length;
        %%  filter response
        [c{iteration_n, iteration}] = gaussian_kernel(l, sig, n_ord);
        h_response{iteration_n, iteration} = fftshift(abs(fft((c{iteration_n, iteration}), n_fft)));
        legend_string{iteration_n, iteration} = ['l=', num2str(l), ', \sigma=', num2str(sig)];
...
for iteration_n = 1: numel(n_ords)
    ...
    xlabel('\omega')
    title(['Derivative order n=', num2str(n_ords(iteration_n))])
